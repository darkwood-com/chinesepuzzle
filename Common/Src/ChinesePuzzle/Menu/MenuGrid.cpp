...
MenuGridSwipeTo* MenuGridSwipeTo::actionWithDuration(ccTime duration, const CGFloat& swipe)
{
	MenuGridSwipeTo *pMenuGridSwipeTo = new MenuGridSwipeTo();
	pMenuGridSwipeTo->initWithDuration(duration, swipe);
	pMenuGridSwipeTo->autorelease();
	
	return pMenuGridSwipeTo;
}

bool MenuGridSwipeTo::initWithDuration(ccTime duration, const CGFloat& swipe)
{
...
CCObject* MenuGridSwipeTo::copyWithZone(CCZone *pZone)
{
	CCZone* pNewZone = NULL;
	MenuGridSwipeTo* pCopy = NULL;
	if(pZone && pZone->m_pCopyObject) 
	{
		//in case of being called at sub class
		pCopy = (MenuGridSwipeTo*)(pZone->m_pCopyObject);
	}
	else
	{
		pCopy = new MenuGridSwipeTo();
...
void MenuGridSwipeTo::startWithTarget(CCNode *pTarget)
{
	CCActionInterval::startWithTarget(pTarget);
	m_startSwipe = ((MenuGrid*) m_pTarget)->getSwipe();
	m_delta = m_endSwipe - m_startSwipe;
}

void MenuGridSwipeTo::update(ccTime time)
{
	if (m_pTarget)
	{
		((MenuGrid*) m_pTarget)->setSwipe(m_startSwipe + m_delta * time);
	}
}
...
bool MenuGrid::initWithContentSize(const CCSize& size)
{
	if (this->init())
	{
...
void MenuGrid::setItems(CCArray* items)
{
...
CCArray* MenuGrid::getItems()
{
	return this->items;
}

void MenuGrid::setGridSize(ccGridSize gridSize)
{
...
ccGridSize MenuGrid::getGridSize()
{
	return this->gridSize;
}

void MenuGrid::setSwipe(CGFloat swipe)
{
...
CGFloat MenuGrid::getSwipe()
{
	return this->m_fOffsetSwipe;
}

void MenuGrid::setPage(int page)
{
...
int MenuGrid::getPage()
{
	return this->page;
}

int MenuGrid::getMaxPage()
{
	return (int) ceil((float) this->items->count() / (this->gridSize.x * this->gridSize.y));
}

void MenuGrid::swipeToPage(int page)
{
	if(page >= 0 && page < this->getMaxPage())
	{
		runAction(CCSequence::actionOneTwo(
			MenuGridSwipeTo::actionWithDuration(0.3f, (this->page - page) * this->getContentSize().width),
			CCCallFunc::actionWithTarget(this, callfunc_selector(MenuGrid::swipeToPageEnded))
		));
	}
}

void MenuGrid::swipeToPageEnded()
{
...
void MenuGrid::setContentSize(const cocos2d::CCSize& size)
{
...
void MenuGrid::ccTouchMoved(CCTouch* pTouch, CCEvent* pEvent)
{
...
void MenuGrid::ccTouchEnded(CCTouch* pTouch, CCEvent* pEvent)
{
	if(m_pScrollTouch != pTouch)
		return;
...
void MenuGrid::ccTouchCancelled(CCTouch* pTouch, CCEvent* pEvent)
{
	if (m_pScrollTouch == pTouch)
	{
		m_pScrollTouch = NULL;
